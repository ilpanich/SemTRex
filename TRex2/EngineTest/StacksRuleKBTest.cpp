...
bool trex_testing::TestKBSequenceWithExtParams() {
	RulePkt *pkt = new RulePkt(true);
	Constraint c1[1];
	Constraint c2[1];
	Constraint c3[1];
	...
	buildConstraint(c3, 3);
	pkt->addPredicate(3, c3, 1, 1, win, kind);
	pkt->addKBRootPredicate(NULL,0,"/home/lele/git/SemTRex/rdf3x-0.3.5/bin/db", "select ?name where { ?p <isCalled> ?name. ?p <bornInLocation> &city }");
	ExtParameter * ep;
	...
	attPkt1[0] = att1;
	attPkt1[1] = att3;
	PubPkt *pkt2 = new PubPkt(2, attPkt1, 2);
	pkt2->setTime(2);
	att1.intVal = 1;
	Attribute attPkt[2];
	attPkt[0] = att1;
