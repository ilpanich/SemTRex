...
PubPkt::PubPkt(int parEventType, Attribute *parAttributes, int parAttributesNum) {
...
PubPkt::PubPkt(const PubPkt &pkt) {
	...
	attributes = new Attribute[attributesNum];
	for (int i=0; i<attributesNum; i++) attributes[i]=pkt.attributes[i];
...
PubPkt::~PubPkt() {
  ...
  #if LOG==1
	count--;
	if (count == 0) cout << "* All messages have been successfully deleted *" << endl;
  #endif
}
...
int PubPkt::getIntAttributeVal(int index) {
	return attributes[index].intVal;
}
