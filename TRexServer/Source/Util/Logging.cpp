...
void Logging::init(){
	...
	boost::shared_ptr< std::ostream > pConsoleStream(&std::cout, logging::empty_deleter());
	...
	pBackend->set_formatter(fmt::stream
		<< "[" << fmt::date_time< boost::posix_time::ptime >("TimeStamp", "%d.%m.%Y %H:%M:%S")
		<< "] " << fmt::message());

	// Add a filter on severity level:
	// Write all records with "info" severity or higher
	pSink->set_filter(
		flt::attr< severity_level >("Severity") >= info
	);
}
