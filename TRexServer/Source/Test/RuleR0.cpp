...
RulePkt* RuleR0::buildRule(){
	...
	OpTree* areaOpTree= new OpTree(new RulePktValueReference(indexPredTemp, ATTR_AREA), STRING);
	fireTemplate->addAttribute(ATTR_AREA, areaOpTree);
	// MeasuredTemp attribute in template
	OpTree* measuredTempOpTree= new OpTree(new RulePktValueReference(indexPredTemp, ATTR_TEMPVALUE), INT);
