...
RulePkt* RuleR1::buildRule(){
	...
	OpTree* areaOpTree= new OpTree(new RulePktValueReference(indexPredSmoke, RuleR0::ATTR_AREA), STRING);
	fireTemplate->addAttribute(RuleR0::ATTR_AREA, areaOpTree);
	// MeasuredTemp attribute in template
	OpTree* measuredTempOpTree= new OpTree(new RulePktValueReference(indexPredTemp, RuleR0::ATTR_TEMPVALUE), INT);
