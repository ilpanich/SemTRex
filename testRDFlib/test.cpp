...
//	string query = "select ?name ?c where { ?p <isCalled> ?name. ?p <bornInLocation> ?c }";
	string query = "select ?name ?city where { ?p <isCalled> ?name. ?p <bornInLocation> ?city. ?p <isCalled> \"Denis Leary\". ?p <bornInLocation> \"Worcester,_Massachusetts\"}";
	string test = "name";
